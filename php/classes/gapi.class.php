...
class gapi
{
  ...
  const client_login_url = 'https://www.google.com/accounts/AuthSubSessionToken';
   ...
   * @param Int $max_results OPTIONAL: Max results returned
   */
  public function requestReportData($report_id, $dimensions, $metrics, $sort_metric=null, $filter=null, $start_date=null, $end_date=null, $start_index=1, $max_results=20)
  {
  ...
  protected function authenticateUser($email, $password)
  {
    ...
    if(substr($response['code'],0,1) != '2' || !is_array($auth_token) || empty($auth_token['Auth']))
    {
     // throw new Exception('GAPI: Failed to authenticate user. Error: "' . strip_tags($response['body']) . '"');
        echo '<p>Unable to authenticate user. Make sure your Google username, password, and Profile ID are set correct in the On Site Google Analytics Plugin settings</p>';
        return;
    }
